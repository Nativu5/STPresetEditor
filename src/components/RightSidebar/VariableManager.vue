    ...
    <div class="flex-grow border rounded-lg p-3">
      ...
      <div class="space-y-4">
        <Combobox v-model="selectedVariableForRename" nullable>
          <div class="relative">
            <ComboboxLabel class="block text-sm font-medium text-gray-700">Variable to Rename</ComboboxLabel>
            ...
            >
              <ComboboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                <div v-if="filteredVariables.length === 0 && query !== ''" class="relative cursor-default select-none py-2 px-4 text-gray-700">
                  Nothing found.
                </div>
                <ComboboxOption
...
const isRenameValid = computed(() => {
    ...
    if (store.variables[trimmedNewName] && trimmedNewName !== selectedVariableForRename.value) return false;
    return true;
});
